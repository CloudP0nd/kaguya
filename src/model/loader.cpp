...
bool ModelLoader::load(const std::string& path, bool mmap) {
    ...
    std::cout << "Kaguya: Model loaded successfully — "
              << arch_to_string(weights_.arch) << " "
              << weights_.hparams.emb_dim << "d "
              << weights_.hparams.num_layers << "L "
              << weights_.hparams.num_heads << "H\n";

    return true;
}

std::string ModelLoader::arch_name() const {
    return arch_to_string(weights_.arch);
}
...
bool ModelLoader::build_weight_refs() {
    ...
    std::string prefix = arch_to_string(weights_.arch);
    ...
    for (int64_t i = 0; i < hp.num_layers; ++i) {
        ...
        // Attention norm
        int64_t ne0 = 0, ne1 = 0; set_weight(lw.attn_norm, lw.attn_norm_bytes,
                    ne0, ne1,
        ...
        // FFN norm
        ne0 = 0; ne1 = 0; set_weight(lw.ffn_norm, lw.ffn_norm_bytes,
                    ne0, ne1,
        ...
        // MoE gate (router)
        if (hp.num_experts > 0) {
            set_weight(lw.moe_gate, lw.moe_gate_bytes, ne0, ne1,
...
void ModelLoader::print_info() const {
    ...
    const auto& hp = weights_.hparams;
    std::cout << "\n--- Model Structure ---\n";
    std::cout << "Architecture:   " << arch_to_string(weights_.arch) << "\n";
